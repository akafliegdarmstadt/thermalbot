...
class TableAgent:
    ...
    def otos(self, observation):
        ...
        assert bankangle >= np.deg2rad(-45) and bankangle <= np.deg2rad(
            45
        )  # Make sure bankangle is between -45 and +45 degrees
    ...
    def __init__(
        self, learning_rate=0.1, discount=0.9, randomness=0.3, decay=1, deadzone=0.1
    ):
    ...
    def update(self, observation, action, reward, nextobservation):
        ...
        ) * oldq + self.learning_rate * learned
...
class SARSAAgent:
    def otos(self, observation):
        """Convert from observation to indices for our policy."""
        x, y, z, bankangle, phi, dx, dy, dz, dbankangle, dphi, liftgradient = (
            observation
        )
    ...
    def __init__(self, learning_rate=0.9, discount=0.0, randomness=0.3, deadzone=0.1):
        # Learning rate and discount factor are chosen quite randomly
    ...
    def update(self, observation, action, reward, nextobservation):
        ...
        self.policy[dy, bankangle, lg] = thisq + self.learning_rate * (
            reward + self.discount * nextq - thisq
        )
